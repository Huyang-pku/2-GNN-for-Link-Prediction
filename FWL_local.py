...
def compute_D(self_loop = False):
    D_dense = torch.zeros((tailed_size, tailed_size, tailed_size), dtype = torch.bool)
    for t in range(data.edge_index.shape[1]):
    ...
    D = D_dense.to_sparse().to(torch.long)
    torch.save(D.indices(), 'FWL_D_{}.pt'.format(dataset))
    return D.indices()
...
def cut_edge(initial_edge_attr, initial_x, initial_edge, A, B_ob, D, node_mask, pruning, device):
    if two_wl_flow == 'local_dir':
        ...
        output_D = D[:,mask[D[0]*tailed_size+D[2]] | mask[D[1]*tailed_size+D[2]]].to(device)
...
if True:
    if True:
        ...
        '''
        D_full = torch.empty((3,tailed_size**3),dtype = torch.long)
        t = 0
        for i in range(tailed_size):
            for j in range(tailed_size):
                for k in range(tailed_size):
                    D_full[0,t] = i
                    D_full[1,t] = j
                    D_full[2,t] = k
        '''
...
if True:
    ...
    class Net(torch.nn.Module):
        ...
        def forward(self, edge_mask, use_logsoftmax=False):
            ...
            x = torch.cat([x, matmul(x1,x2,data.D).reshape(tailed_size ** 2, -1)], -1)
    ...
    initial_train_edge_attr, initial_train_x, initial_train_edge, A_train, D_train = cut_edge(
        ...
        device)  # , edge2)
    initial_val_edge_attr, initial_val_x, initial_val_edge, A_val, D_val = cut_edge(initial_edge_attr,
                                                                                          initial_x, initial_edge,
                                                                                          A, B, D,
                                                                                          node_mask, True,
                                                                                          device)  # , edge2)
    initial_test_edge_attr, initial_test_x, initial_test_edge, A_test, D_test = cut_edge(initial_edge_attr,
                                                                                              initial_x,
                                                                                              initial_edge,
                                                                                              A, B, D,
                                                                                              test_mask,
...
if True:
    ...
    def initialize_val():
        ...
        data.D = deepcopy(D_val).to(device)
    ...
    def initialize_test():
        ...
        data.D = deepcopy(D_test).to(device)
    ...
    if True:
        ...
        for epoch in tqdm(range(1, 1001)):
            ...
            data.edge_attr, data.x, data.edge_index, data.A, data.D = cut_edge(initial_train_edge_attr,
                                                                                  initial_train_x,
                                                                                  initial_train_edge,
                                                                                  A_train, B_train, D_train,
                                                                                  train_mask, True,
                                                                                  device)
